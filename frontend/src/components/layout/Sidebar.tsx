...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 border-r border-zinc-800/80 bg-zinc-950 flex flex-col justify-between shrink-0">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-emerald-600 via-teal-500 to-cyan-400 p-[1px] shadow-lg shadow-emerald-950/40">
            <div className="w-full h-full bg-zinc-950 rounded-[11px] flex items-center justify-center">
              <Sprout className="w-5 h-5 text-emerald-400 group-hover:scale-110 transition" />
            </div>
          </div>
          <div className="flex flex-col">
            ...
            <span className="text-[11px] text-zinc-400">Plant to Watershed</span>
          </div>
        </Link>

        {/* Navigation list */}
        <nav className="flex flex-col gap-1">
          <span className="text-[10px] font-mono uppercase tracking-wider text-zinc-400 px-3 py-1 font-semibold">
            Módulos del Sistema
          </span>
          {navigation.map((item) => {
            const isAllowed =
      ...
      {/* Footer Info */}
      <div className="p-4 border-t border-zinc-900 text-[11px] text-zinc-400 flex flex-col gap-1.5">
        <div className="flex items-center gap-2">
          <Layers className="w-3.5 h-3.5 text-zinc-400" />
          <span className="font-mono text-zinc-300">Escalas: Micro ⇄ Meso ⇄ Macro</span>
        </div>
