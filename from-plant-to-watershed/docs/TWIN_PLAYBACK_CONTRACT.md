...
El lector compara el SHA-256 del SQLite completo con el manifiesto antes de servir una página. Cachea el resultado solo mientras dispositivo, inodo, tamaño, `mtime_ns` y `ctime_ns` permanezcan iguales; si el archivo cambia, vuelve a calcular el hash. También valida el checksum de cada registro leído. Un fallo de integridad responde 503. El nombre único permite reintentar la publicación tras una falla antes del commit PostgreSQL sin sobrescribir un sidecar ya publicado. Un intento fallido puede dejar un archivo huérfano sin referencia; la limpieza administrativa debe consultar los manifiestos persistidos antes de retirarlo. Los artefactos referenciados permanecen inmutables.
